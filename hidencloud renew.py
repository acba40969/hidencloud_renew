...
class HidenCloud:
    def __init__(self, cookie_str, tg_config=None, proxy=None):
        ...
        if self.proxy:
            ...
            logger.info(f"已启用代理出口: {self.mask_proxy(self.proxy)}")

        self.parse_and_set_cookies()

    @staticmethod
    def mask_proxy(proxy_url):
        """打印日志时隐藏代理的账号密码，避免泄露到日志"""
        try:
            if '@' in proxy_url:
...
def main():
    ...
    # socks5h://1.2.3.4:1080  (h 后缀代表由代理端解析域名，推荐使用)
    ...
    # 代理优先级：环境变量 SOCKS_PROXY > config.json 中的 proxy 字段
    if not proxy:
        proxy = config.get("proxy")
    if proxy:
        logger.info("检测到代理配置，所有请求将通过代理转发")

    for cookie_str in account_cookies:
        if not cookie_str.strip(): continue
        try:
            bot = HidenCloud(cookie_str, tg_config, proxy=proxy)
            bot.run_task()
        except Exception as e:
